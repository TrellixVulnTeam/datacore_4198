(function( $ ){
    ...
    function __changeHandler( e ) {
		...
		if (window.treeview_changed) {
			treeview_changed()
		}
    }
	
	function __arrowClickHandler( e ) {
        var currentTarget = $(this);
		checkbox_sel = currentTarget.parent().children('input').first();
		
		if(currentTarget.hasClass('fa-caret-down')){
			checkbox_sel.parent()
                   .find('ul.collapse')
                   .collapse('hide')
                   .parent()
        ...
        me.trigger('treeview.change', checkbox_sel, me);
    ...
    function _init() {
        ...
        me.find('li').each(function( index, element ) {
            ...
            elmt.prepend(chk);
			if(elmt.hasClass('root'))
				elmt.prepend(
					$('<i></i>').addClass(settings.css.listItem + ' ' + settings.css.collapsed)
				);
		...
		me.on('click', '.fa-li', __arrowClickHandler);
    }
